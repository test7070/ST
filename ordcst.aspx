	...
	<head>
		...
		<script type="text/javascript">
			...
			aPop = new Array(
				...
				,['txtCustno__', 'btnCust__', 'cust', 'noa,nick,comp', 'txtCustno__,txtCust__', 'cust_b.aspx']
			);
			brwCount2 = 11;
				...
				switch (t_name) {
					...
					case 'ordcs':
						...
						if (ordbsArray[0] != undefined) {
							...
							if(q_getPara('sys.project').toUpperCase()=='PK')
								ret = q_gridAddRow(bbsHtm, 'tbbs', 'txtProductno,txtProduct,txtSpec,txtDime,txtWidth,txtLengthb,txtRadius,txtOrdbno,txtNo3,txtPrice,txtMount,txtWeight,txtTotal,txtMemo,txtTheory,txtStyle,txtClass,txtUno,txtSize,txtUnit,txtUnit2,txtUcolor,txtScolor,txtSource', ordbsArray.length, ordbsArray, 'productno,product,spec,dime,width,lengthb,radius,noa,no3,price,mount,weight,total,memo,theory,style,class,uno,size,unit,unit2,ucolor,scolor,source', 'txtProductno,txtProduct,txtSpec');
							else
                ...
                for (var i = 0; i < q_bbtCount; i++) {
                    ...
                    if (!$('#btnMinut__' + i).hasClass('isAssign')) {
                    	...
                    	$('#txtCustno__' + i).bind('contextmenu', function(e) {
                            /*滑鼠右鍵*/
                            e.preventDefault();
                            var n = $(this).attr('id').replace('txtCustno__', '');
                            $('#btnCust__'+n).click();
                        });
                    }
                }
			...
			function genNo2(){
				...
				var maxno2 = 0;
				for(var i=0;i<q_bbsCount;i++){
					maxno2 = q_float('txtNo2_'+i)>maxno2?q_float('txtNo2_'+i):maxno2;
				}
				//console.log(maxno2+"__"+q_bbsCount);
				for(var i=0;i<q_bbsCount;i++){
					
					if($('#txtNo2_'+i).val().length==0){
						maxno2++;
						t_no2 = '000'+maxno2;
						t_no2 = t_no2.substring(t_no2.length-3,t_no2.length);
						//console.log(q_float(t_no2));
						$('#txtNo2_'+i).val(t_no2);
						//$('#txtNo2_'+i).val(maxno2);
					}
				}
			}
			...
			function bbsSave(as) {
				if (!as['productno'] && !as['product'] && !as['spec'] && !as['size'] && !dec(as['total'])) {//不存檔條件
					as[bbsKey[1]] = '';
					/// no2 為空，不存檔
					return;
				}
		...
		<div class='dbbs'>
			<table id="tbbs" class='tbbs' style=' text-align:center'>
				...
				<tr style='background:#cad3ff;'>
					...
					<td>
						<input class="st" id="txtProductno.*" type="text" style="width:95%;" />
						<input class="btn" id="btnProduct.*" type="button" style="display:none;"/>
						<input class="bcc" type="text" id="txtProductno1.*"  style="width:95%;"/>
            ...
            <table id="tbbt" class='tbbt'  border="2"  cellpadding='2' cellspacing='1'>
                ...
                <tr>
                    ...
                    <td>
                    	...
                    	<input type="button" id="btnCust..*" value="." style="display:none;">
                    </td>
