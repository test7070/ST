...
<head>
    ...
    <script type="text/javascript">
        ...
        var q_readonlys = [];
        var bbmNum = [];  
        var bbsNum = [['textSize1', 10, 3, 1],['textSize2', 10, 2, 1],['textSize3', 10, 3, 1],['textSize4', 10, 2, 1],['txtRadius', 10, 3, 1],['txtWidth', 10, 2, 1],['txtDime', 10, 3, 1],['txtLengthb', 10, 2, 1],['txtMount', 10, 2, 1],['txtPrice', 10, 2, 1],['txtTotal', 10, 0, 1],['txtEweight', 10, 3, 1],['txtAdjweight', 10, 3, 1],['txtEweight2', 10, 3, 1]];
        var bbmMask = [];
        var bbsMask = [];
        ...
        function sum() {
            ...
            for (var j = 0; j < q_bbsCount; j++) {
				q_tr('txtTotal_'+j,q_mul(q_float('txtPrice_'+j),q_float('txtMount_'+j)));
            }  // j
        }
        ...
        function bbsAssign() { 
            for (var j = 0; j < ( q_bbsCount==0 ? 1 : q_bbsCount); j++) {
				...
				$('#txtProductno_' + j).change(function() {
					var n = $(this).attr('id').split('_')[$(this).attr('id').split('_').length-1];
					if($('#txtProductno_'+n).val().substr(0,1)=='W' && $('#txtProduct_'+n).val().indexOf('廢料')>-1)
						$('#txtUno_'+n).val($('#txtProductno_'+n).val());
				});
            }
            _bbsAssign();
            size_change();
