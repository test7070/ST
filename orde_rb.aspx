...
<html xmlns="http://www.w3.org/1999/xhtml" dir="ltr">
	<head>
		...
		<script type="text/javascript">
			...
			q_tables = 's';
			var q_name = "orde";
			var q_readonly = ['txtNoa', 'txtWorker', 'txtWorker2', 'txtComp', 'txtAcomp', 'txtMoney', 'txtTax', 'txtTotal', 'txtTotalus', 'txtSales', 'txtOrdbno', 'txtOrdcno','txtVccno'];
			...
			function mainPost() {
				...
				$('#txtVccno').click(function(){
					...
					if (t_vccno.length > 0) {
						t_where = "noa='" + t_vccno + "'";
						q_box("vcc_rb.aspx?" + r_userno + ";" + r_name + ";" + q_time + ";" + t_where, 'vcc', "95%", "95%", "出貨單");
					}
				});
					...
					if(emp($('#txtVccno').val()) && !check_stkucc && $('#txtPostname').val() == '001'){
						...
						if(checkstktmp.length>0){
							...
							for(var j=0; j<checkstktmp.length; j++){
								...
								q_gt('view_ordes', t_where, 0, 0, 0, "btnOVchecknotv_"+j, r_accy);
							}
							return;
						}else{ // 表身無資料
							check_stkucc=true;
				...
				$('#btnAddr2').mousedown(function(e) {
					...
					var t_consignee = $('#txtMemo2').val().split(';');
					...
					for (var i = 0; i < maxline; i++) {
						...
						tr.innerHTML+="<td id='addr2_tdConsignee_"+i+"'><input id='addr2_txtConsignee_"+i+"' type='text' class='txt c1 addr2' value='"+t_consignee[i]+"' /></td>";
				...
				$('#btnAddr_plus').click(function() {
					...
					var tr = document.createElement("tr");
						...
						tr.innerHTML+="<td id='addr2_tdConsignee_"+rowslength+"'><input id='addr2_txtConsignee_"+rowslength+"' type='text' class='txt c1 addr2' value='' /></td>";
						var tmp = document.getElementById("addr2_close");
						tmp.parentNode.insertBefore(tr,tmp);
				});
			...
			function q_gtPost(t_name) {
				...
				if (t_name.substr(0,14)=='btnOVchecknotv'){
					var n = t_name.split('_')[1];
					var as = _q_appendData("view_ordes", "", true);
					var tnotv=0;
					for (var i = 0; i < as.length; i++) {
						tnotv=q_add(tnotv,dec(as[i].notv))
					}
					checkstktmp[n].unvcc=tnotv;
					check_stk_notv--;
				}
				//庫存資料接收完畢 判斷庫存是否低於安全存量
				if((t_name.substr(0,13)=='btnOVcheckstk' || t_name.substr(0,14)=='btnOVchecknotv') && check_stk==0 && check_stk_notv==0 ){
					var x_err='';
					for(var j=0; j<checkstktmp.length; j++){
			...
			function _btnSeek() {
				if (q_cur > 0 && q_cur < 4)
					return;
				q_box('orde_rb_s.aspx', q_name + '_s', "500px", "400px", q_getMsg("popSeek"));
			}
	...
	<body>
		...
		<div id="div_addr2" style="position:absolute; top:244px; left:600px; display:none; width:530px; background-color: #CDFFCE; border: 5px solid gray;">
			<table id="table_addr2" style="width:100%;" border="1" cellpadding='2' cellspacing='0'>
				<tr>
					...
					<td style="width:100px;background-color: #f8d463;" align="center">收件人</td>
				</tr>
				<tr id='addr2_close'>
			...
			<div class="dview" id="dview">
				<table class="tview" id="tview">
					<tr>
						...
						<td align="center" style="width:10%"><a id='vewVcc'>出貨</a></td>
					</tr>
					<tr>
